...
async function gandu() {
  try {
    ...
    if (gnome && gnome.trim().toLowerCase() !== lund.toLowerCase()) {
      ...
      console.log(`${endi}`)
      console.log(chalk.bgBlack(chalk.redBright('Starting Toji md bot')))
    }
  } catch (error) {
    console.error('Error:', error)
  }
...
if (pairingCode && !conn.authState.creds.registered) {
  ...
  if (!!global.pairingNumber) {
    ...
    if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
      console.log(
        chalk.bgBlack(chalk.redBright("Start with your country's WhatsApp code, Example : 263xxx"))
      )
...
async function connectionUpdate(update) {
  ...
  if (connection === 'open') {
    ...
    const msg = `Toji md \nsup🍼 ${name}, Congrats you have successfully deployed TOJI MD\n  ✅ *Bot Connection Status:* \n     *Toji MD* successfully connected to this device! \n\n  ⚙️ *Prefix:*  \n\n  🥪 *Creator:* *king Johan* \n  🏢 *Organization:* *king johan tech .* \n  🤸 *Updated:* *2024* \n\n  🍼 *Join our WhatsApp Channel for updates:* \n  https://whatsapp.com/channel/0029VanxGm5J3jv39RvJIM39 \n\n  🔄 *Stay tuned for upcoming features!* \n\nTOJI MD`
